...
def load_crisis_prompts():
    try:
        crisis_prompt_path = get_abs_path(prompts_conf["crisis_prompt_path"])
    except KeyError as e:
        logger.error(f"[load_crisis_prompts]在yaml配置项中没有crisis_prompt_path配置项")
        raise e

    try:
        return open(crisis_prompt_path, "r", encoding="utf-8").read()
    except Exception as e:
        logger.error(f"[load_crisis_prompts]解析危机应对提示词出错，{str(e)}")
        raise e
